...
public static class Activations
{
    ...
    public static double Sigmoid(this double value) => 1 / (1 + Math.Exp(-value));

    public static double LeakyRelu(this double value, double alpha = 0.3) => (value >= 0) ? value : (value * alpha);

    public static double Softplus(this double value, double alpha = 0.3) => (1 + Math.Exp(value)).Log();
}
